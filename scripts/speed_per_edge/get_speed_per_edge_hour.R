...
df_speed_per_edge_hour <- dbGetQuery(con, "SELECT way_id, session_id, hour, total_distance, total_time, total_distance / total_time AS speed
                                FROM (
                                SELECT way_id, session_id, extract(hour from to_timestamp(seconds)) AS hour, SUM(ST_Distance(geo, nextGeo)) AS total_distance, (MAX(seconds+(millis/1000.0)) - MIN(seconds+(millis/1000.0))) AS total_time
                                FROM (
                                ...
                                GROUP BY session_id, way_id, hour
                                ) T4
                                WHERE total_time > 0
                                ORDER BY speed DESC")
