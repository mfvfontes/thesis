...
list_points_edge_osm <- df_points_per_edge[1:500, 1]

fun.ecdf <- ecdf(df_points_per_edge$points)

my.ecdf <- fun.ecdf(sort(df_points_per_edge$points))

my_ecdf_df <- data.frame(my.ecdf)

my_ecdf_df$points <- sort(df_points_per_edge$points)

tail(my_ecdf_df, 300)

P <- ecdf(df_points_per_edge$points)
grid()
plot(P, log="x", xlim=c(1, max(df_points_per_edge$points)))
...
for(way_id in list_points_edge_osm) {
  ...
  if(length(parsed_line) != 0) {
    ...
    if(df_points_per_edge[counter, 2] >= 4258) {
      
      m <- addPolylines(m, lons, lats, color='red')
      
    } else if (df_points_per_edge[counter, 2] >= 1272 && df_points_per_edge[counter, 2] < 4258) {
