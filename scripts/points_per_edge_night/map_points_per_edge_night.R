...
list_points_per_edge_night <- df_points_per_edge_night[1:500, 1]

fun.ecdf <- ecdf(df_points_per_edge_night$points)

my.ecdf <- fun.ecdf(sort(df_points_per_edge_night$points))

my_ecdf_df <- data.frame(my.ecdf)

my_ecdf_df$points <- sort(df_points_per_edge_night$points)

tail(my_ecdf_df, 500)

P <- ecdf(df_points_per_edge_night$points)
grid()
#plot(P, log="x", xlim=c(1, max(df_points_per_edge_night$points)))
...
for(way_id in list_points_per_edge_night) {
  ...
  if(length(parsed_line) != 0) {
    ...
    if(df_points_per_edge_night[counter, 2] >= 7705) {
      
      m <- addPolylines(m, lons, lats, color='red')
      
    } else if (df_points_per_edge_night[counter, 2] >= 1705 && df_points_per_edge_night[counter, 2] < 7705) {
